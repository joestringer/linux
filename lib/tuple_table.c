...
static struct flex_array *alloc_buckets(unsigned int n_buckets, gfp_t gfp)
{
	...
	err = flex_array_prealloc(buckets, 0, n_buckets, gfp);
	if (err) {
...
{
	struct table_instance *ti = kmalloc(sizeof(*ti), gfp);

	if (!ti)
		return NULL;

	ti->buckets = alloc_buckets(n_elements, gfp);
	if (!ti->buckets) {
		kfree(ti);
		return NULL;
	}
...
static int ts_range_n_bytes(const struct ts_range *range)
{
...
EXPORT_SYMBOL(ts_mask_key);

bool ts_compare(const void *key1, const void *key2,
		const struct ts_range *range)
{
	     ...
	     rcu_callback_t freefn, gfp_t gfp)
{
	...
	ti = table_instance_alloc(n_buckets, gfp);
	if (!ti)
		return -ENOMEM;

	masked_key = __alloc_percpu(key_len, __alignof__(key_len));
	if (!masked_key)
		goto free_ti;

	rcu_assign_pointer(table->ti, ti);
	INIT_LIST_HEAD(&table->mask_list);
	table->masked_key = masked_key;
...
void tst_destroy(struct ts_table *table)
{
	...
	free_percpu(table->masked_key);
	if (ti) {
		table_instance_destroy_and_free(ti, false, table->free);
	}
}
EXPORT_SYMBOL(tst_destroy);

/* Caller must acquire mutex write lock. */
int tst_flush(struct ts_table *table)
{
	struct list_head mask_list = table->mask_list;
	struct table_instance *ti, *old_ti;
	struct ts_mask *mask;

	old_ti = rcu_dereference_protected(table->ti, 1);
	ti = table_instance_alloc(old_ti->n_buckets, table->gfp);
	if (!ti)
		return -ENOMEM;
...
{
	struct table_instance *ti = rcu_dereference(table->ti);
	...
	ver = ti->node_ver;
	while (ctx->bucket < ti->n_buckets) {
		i = 0;
		head = flex_array_get(ti->buckets, ctx->bucket);
...
static struct table_instance *table_instance_rehash(struct table_instance *ti,
						    int n_buckets, gfp_t gfp)
{
	struct table_instance *new_ti;

	new_ti = table_instance_alloc(n_buckets, gfp);
	if (!new_ti)
		return NULL;
...
static struct table_instance *table_instance_expand(struct table_instance *ti,
						    gfp_t gfp)
{
	return table_instance_rehash(ti, ti->n_buckets * 2, gfp);
}
...
static struct ts_element *__tst_masked_lookup(struct ts_table *table,
					      const void *unmasked,
					      const void *mask,
					      const struct ts_range *range)
{
	struct table_instance *ti = rcu_dereference_check(table->ti, 1);
	struct ts_element *element;
	struct hlist_head *head;
	void *masked_key;
	u32 hash;

	masked_key = this_cpu_ptr(table->masked_key);
	ts_mask_key(masked_key, unmasked, mask, range);
	...
	hlist_for_each_entry_rcu(element, head, node[ti->node_ver]) {
		if (&element->mask->key == mask && element->hash == hash &&
		    ts_compare(element->key, masked_key, range))
			return element;
	}
	return NULL;
}
				     ...
				     const struct ts_range *range)
{
	...
	list_for_each_entry_rcu(tmask, &table->mask_list, list) {
		if (!memcmp(range, &tmask->range, sizeof(*range)) &&
		    ts_compare(mask, &tmask->key, range)) {
...
{
	struct table_instance *ti = rcu_dereference_protected(table->ti, 1);
...
static struct ts_mask *mask_alloc(const void *mask, size_t mask_len,
				  const struct ts_range *range, gfp_t gfp)
{
	struct ts_mask *tm = kmalloc(sizeof *tm + mask_len, gfp);

	if (tm) {
		memcpy(&tm->key, mask, mask_len);
		tm->range = *range;
		tm->ref_count = 1;
		tm->key_len = mask_len;
	}
...
static struct ts_mask *tst_mask_find__(const struct ts_table *tbl,
				       const struct ts_mask *mask)
{
	struct list_head *ml;

	list_for_each(ml, &tbl->mask_list) {
		struct ts_mask *m = container_of(ml, struct ts_mask, list);
...
static int ts_mask_insert(struct ts_table *tbl, struct ts_element *e,
			  const struct ts_mask *new)
{
	struct ts_mask *mask;

	mask = tst_mask_find__(tbl, new);
	if (!mask) {
		/* Allocate a new mask if none exists. */
		mask = mask_alloc(new->key, new->key_len, &new->range,
				  tbl->gfp);
		if (!mask)
			return -ENOMEM;
...
struct ts_mask *tst_lookup_mask(const struct ts_table *tbl, const void *mask)
{
	...
	list_for_each(ml, &tbl->mask_list) {
		struct ts_mask *tm = container_of(ml, struct ts_mask, list);
		const u8 *m = (const u8 *)&tm->key + tm->range.start;

		if (!memcmp(mask, m, ts_range_n_bytes(&tm->range))) {
			return tm;
		}
	}
			 ...
			 void *key)
{
	...
	ti = rcu_dereference_protected(table->ti, 1);
	table_instance_insert(ti, e);
	table->count++;

	/* Expand table, if necessary, to make room. */
	if (table->count > ti->n_buckets)
		new_ti = table_instance_expand(ti, table->gfp);
	else if (time_after(jiffies, table->last_rehash + REHASH_INTERVAL))
		new_ti = table_instance_rehash(ti, ti->n_buckets, table->gfp);
	else
		return;
	       ...
	       const struct ts_mask *mask)
{
	...
	err = ts_mask_insert(table, e, mask);
	if (err)
		return err;
