		       ...
		       bool full, const struct sw_flow_mask *mask)
{
	...
	range.end = full ? mask->head.key_len
			 : mask->head.range.end;
	ts_mask_key(dst, src, &mask->head.key, &range);
}
...
int ovs_flow_tbl_init(struct flow_table *table)
{
	...
	ufid_ti = table_instance_alloc(TBL_MIN_BUCKETS, GFP_KERNEL);
	if (!ufid_ti)
		return -ENOMEM;

	err = tst_init(&table->tt, sizeof(struct sw_flow), TBL_MIN_BUCKETS,
		       rcu_free_flow_callback, GFP_KERNEL);
	if (err)
		goto free_ti;
...
free_ti:
	table_instance_destroy(ufid_ti, false);
	return err;
}
...
static struct table_instance *ufid_table_expand(struct table_instance *ti)
{
	...
	new_ti = table_instance_alloc(ti->n_buckets * 2, GFP_KERNEL);
	if (!new_ti)
		return NULL;
...
int ovs_flow_tbl_flush(struct flow_table *flow_table)
{
	...
	new_ufid_ti = table_instance_alloc(TBL_MIN_BUCKETS, GFP_KERNEL);
	if (!new_ufid_ti)
		return -ENOMEM;
...
err_free_ti:
	table_instance_destroy(new_ufid_ti, false);
	return -ENOMEM;
}
				      ...
				      const struct sw_flow_match *match)
{
	...
	return ts_compare(flow->id.unmasked_key, key, &range);
}
					  ...
					  const struct sw_flow_match *match)
{
	...
	tst_for_each_elem(e, &tbl->tt, match->key) {
		struct sw_flow *flow = container_of(e, struct sw_flow, head);
...
bool ovs_flow_cmp(const struct sw_flow *flow, const struct sw_flow_match *match)
{
	if (ovs_identifier_is_ufid(&flow->id))
		return ts_compare(&flow->key, match->key, &match->range);
